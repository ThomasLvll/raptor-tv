...
function MoviePreview({ movie }) {
    return (
        <Link to={`/movie/${movie.id}`}>
          <Box>
            { movie.poster_path? (
            <Image src={`https://www.themoviedb.org/t/p/w220_and_h330_face${movie.poster_path}`} alt={movie.title} boxSize="100%" objectFit="cover" borderRadius="10px" border=".5px solid white" _hover={{ 
